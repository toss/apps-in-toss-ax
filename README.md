...
│   │   └── app.go
│   ├── docs/              # 문서 관리
│   │   └── docs.go        # 문서 소스 및 데이터 구조
│   ├── docid/             # 문서 ID 생성
│   │   └── docid.go       # SHA-256 기반 ID 생성
│   ├── fetcher/           # HTTP 클라이언트
│   │   └── http.go        # 문서 페칭 유틸리티
│   ├── llms/              # llms.txt 파서
│   │   ├── parser.go      # Markdown 파싱 (goldmark)
│   │   └── reader.go      # 문서 리더
│   ├── mcp/               # MCP 서버 구현
│   │   ├── mcp.go         # 서버 초기화 및 설정
│   │   ├── instructions.md # AI 어시스턴트 가이드라인
│   │   ├── tools_list_docs.go
│   │   ├── tools_get_docs.go
│   │   ├── tools_list_examples.go
│   │   ├── tools_get_example.go
│   │   └── tools_search_docs.go
│   └── search/            # 문서 검색 엔진
│       ├── indexer.go     # 문서 인덱싱
│       ├── searcher.go    # 검색 로직
│       ├── parser.go      # 검색 쿼리 파싱
...
│   ├── publish/           # 패키지 매니저 배포
│   │   ├── main.go        # 배포 스크립트
│   │   └── templates/     # 패키지 매니저 템플릿
│   │       ├── ax.rb      # Homebrew formula
│   │       └── ax.json    # Scoop manifest
│   └── shared/            # 공유 유틸리티
│       └── github.go      # GitHub API 클라이언트
│
├── scripts/
│   └── postinstall.js     # npm postinstall 스크립트
│
├── main.go                # 메인 진입점
├── go.mod                 # Go 모듈 정의
├── Makefile               # 빌드 명령어
