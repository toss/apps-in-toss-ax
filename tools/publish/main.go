...
import (
	...
	"github.com/toss/apps-in-toss-ax/pkg/fetcher"
	"github.com/toss/apps-in-toss-ax/tools/shared"
)
...
func publishPackages(ctx context.Context, version string) error {
	...
	config.FormulaName = "Ax"
	config.Description = "AppsInToss CLI"
	client := utils.GetGitHubClient(ctx)
	if err := updatePackage(ctx, client, HOMEBREW_REPO, "ax.rb", brewFormulaTemplate, config); err != nil {
		return err
	}
	return updatePackage(ctx, client, SCOOP_REPO, "ax.json", scoopBucketTemplate, config)
}
...
func fetchConfig(ctx context.Context, version string) (PackageConfig, error) {
	...
	checkPath := fmt.Sprintf("/%s/%s/releases/download/v%[3]s/ax_%[3]s_checksums.txt",
		utils.CLI_OWNER,
		utils.CLI_REPO,
		version,
	)
...
func updatePackage(ctx context.Context, client *github.Client, repo, path string, tmpl *template.Template, config PackageConfig) error {
	...
	branch := "release/ax"
	master := "main"
	if err := shared.CreateGitBranch(ctx, client, OWNER, repo, branch, master); err != nil {
		return err
	}
	// Get file SHA
	...
	resp, _, err := client.Repositories.UpdateFile(ctx, OWNER, repo, path, &commit)
	if err != nil {
		return err
	}
	...
	return shared.CreatePullRequest(ctx, client, OWNER, repo, pr)
}
